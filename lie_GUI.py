...
factcheck = "This is a transcript of a video, check for false comments otr statements, check for sarcasm, check for questions(if exists , then answer them) :\n"
...
#image transform
import skimage
from audiodetection import deception_prediction
import whisper


filename2 = 'path_to_your_video_file.mp4'  # Provide the path to your video file
audio_file_path = './audiodataset/testing_data/output_audio.wav'  # Path to save the audio file
model_path = 'svm_model.pkl'  # Path to pre-trained SVM model
...
SVM_model = load('./model/SVM_model/se_res50+EU_v2/se_res50+EU/split_svc_acc0.735.joblib')
...
class show(QThread):
    ...
    def pred(self):
        ...
        result=int(results) # Lie_pred
        return AU_list, pred_score, result
    def run(self):
        logps,  pred_score, result  = self.pred()
        self.trigger.emit(logps,  pred_score.tolist(), result)
...
class lie_GUI(QDialog, ui.Ui_Dialog):
    ...
    def get_image_file(self):
        ...
        filename2=file_name
        prediction = deception_prediction(audio_file_path, filename2, model_path)    
        if prediction==1:
            print("Truth Audio")
        else:
            print("Deceptive Audio")
        model = whisper.load_model("medium")
        result = model.transcribe("./audiodataset/testing_data/output_audio.wav")
        print("\nTranscript :")
        print(result["text"])
        print("\n\nFact Check :")    
        
        model = genai.GenerativeModel('models/gemini-1.0-pro-latest')
        combined_text = factcheck + "\n" + result["text"]
    ...
    def update_frame(self):
        ...
        #影片讀條
        if self.mode == 'video':
            self.videoprogress.setValue(int(round(self.countframe / self.frame_total ,2) * 100 ))
        ...
        if self.mode =='video' :
            if self.countframe == self.frame_total:
                ...
                if self.lie_count != 0:
                    ...
                    self.prob_label.setText('The probability of deception: {:.0f}% '.format(lie_prob))
                self.timer.stop()
    def display_feature(self, logps, pred_score, result):
...
if __name__=='__main__':
    ...
    window.show()
    sys.exit(app.exec_())
    face_list = []  # Initialize or populate face_list as needed
lie_gui_instance = lie_GUI(face_list)